...
export default function AdminkaPasswordActivationsPage() {
      ...
      <div className="max-w-full sm:max-w-3xl lg:max-w-6xl xl:max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-lg p-3 sm:p-6 mb-3 sm:mb-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold">🔐 Jelszó aktiválások</h1>
              <p className="text-xs sm:text-sm text-gray-600 mt-1">
                Felhasználók, akik megváltoztatták a jelszavukat
              </p>
            </div>
            <button
        ...
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          <div className="flex border-b">
            ...
            >
              ✅ Aktiválva ({activatedUsers.length})
            </button>
            <button
            ...
            >
              ⏳ Nem aktivált ({notActivatedUsers.length})
            </button>
          </div>
                  ...
                  <>
                    {activatedUsers.length === 0 ? (
                      <div className="text-center py-8 text-gray-500">
                        Még senki nem aktiválta a jelszavát
                      </div>
                    ) : (
                      <>
                        {/* Mobile View - Cards */}
                        <div className="sm:hidden space-y-3">
                          {activatedUsers.map(u => (
                            <div key={u.id} className="bg-green-50 rounded-lg p-3 border border-green-200">
                              <div className="font-medium text-sm mb-1">{u.displayName || u.email}</div>
                              <div className="text-xs text-gray-600 mb-2">{u.email}</div>
                              <div className="flex justify-between items-center">
                                <span className="text-xs text-green-700">
                                  ✅ {formatDate(u.passwordActivatedAt)}
                                </span>
                                {u.pharmagisterRole && (
                          ...
                          <table className="w-full">
                            <thead>
                              <tr className="border-b">
                                <th className="text-left py-3 px-4 text-sm">Felhasználó</th>
                                <th className="text-left py-3 px-4 text-sm">Email</th>
                                <th className="text-left py-3 px-4 text-sm">Szerep</th>
                                <th className="text-left py-3 px-4 text-sm">Aktiválás ideje</th>
                              </tr>
                            </thead>
                            <tbody>
                              {activatedUsers.map(u => (
                ...
                {activeTab === 'notActivated' && (
                  <>
                    {notActivatedUsers.length === 0 ? (
                      <div className="text-center py-8 text-gray-500">
                        Minden felhasználó aktiválta a jelszavát! 🎉
                      </div>
                    ) : (
                      <>
                        {/* Mobile View - Cards */}
                        <div className="sm:hidden space-y-3">
                          {notActivatedUsers.map(u => (
                            <div key={u.id} className="bg-orange-50 rounded-lg p-3 border border-orange-200">
                              <div className="font-medium text-sm mb-1">{u.displayName || u.email}</div>
                              <div className="text-xs text-gray-600 mb-2">{u.email}</div>
                              <div className="flex justify-between items-center">
                                <span className="text-xs text-orange-700">
                                  ⏳ Regisztráció: {u.createdAt ? new Date(u.createdAt).toLocaleDateString('hu-HU') : '-'}
                                </span>
                                {u.pharmagisterRole && (
                                  <span className="bg-purple-100 text-purple-800 px-2 py-0.5 rounded text-xs">
                                    {u.pharmagisterRole}
                          ...
                          <table className="w-full">
                            <thead>
                              <tr className="border-b">
                                ...
                                <th className="text-left py-3 px-4 text-sm">Státusz</th>
                              </tr>
                            </thead>
                            <tbody>
                              {notActivatedUsers.map(u => (
