...
export async function getScheduleManagerAccess(authUser, db) {
  ...
  return {
    ...
    isEmployee: ['employee', 'pharmacist', 'assistant', 'pka'].includes(role),
  };
}
...
async function canSendCommunityNotification({ authUser, db, targetUserId, type, notificationData }) {
  if (!COMMUNITY_NOTIFICATION_TYPES.has(type)) return false;
  ...
  if (type === 'community_post_reaction') {
    if (notificationData?.reactorUserId !== authUser.uid) return false;
    return Boolean(post.userId) && post.userId === targetUserId && authUser.uid !== post.userId;
  }

  if (type === 'community_post_comment') {
    if (notificationData?.commenterUserId !== authUser.uid) return false;
    return Boolean(post.userId) && post.userId === targetUserId && authUser.uid !== post.userId;
  }
  ...
  if (type === 'community_comment_reply') {
    ...
    const parentCommentId = notificationData?.parentCommentId;
    if (!parentCommentId) return false;

    const parentSnapshot = await db.collection('communityPosts').doc(postId)
      .collection('comments').doc(parentCommentId).get();
    if (!parentSnapshot.exists) return false;

    const parent = parentSnapshot.data() || {};
    return Boolean(parent.userId) && parent.userId === targetUserId && authUser.uid !== parent.userId;
  }
...
export async function canSendNotificationToUser({ authUser, db, targetUserId, type, tag, url, notificationData }) {
  ...
  if (COMMUNITY_NOTIFICATION_TYPES.has(normalizedType)) {
    return canSendCommunityNotification({
      authUser,
      db,
      targetUserId,
      type: normalizedType,
