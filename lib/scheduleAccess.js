...
async function canSendCommunityNotification({ authUser, db, targetUserId, type, notificationData }) {
  ...
  if (!post.userId || post.userId !== targetUserId) return false;

  // A feladó lehet a poszt reakciózója/kommentelője, de nem maga a tulajdonos.
  return authUser.uid !== post.userId;
}
