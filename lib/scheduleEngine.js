...
function normalizeEmployee(employee) {
  return {
    ...
    weeklyHoursLimit: Number(employee.weeklyHoursLimit || employee.maxWeeklyHours || (() => {
      const ch = Number(employee.contractHours || 0);
      if (ch === 4) return 20;
      if (ch === 6) return 30;
      if (ch === 12) return 48;
      if (ch > 0) return Math.min(ch * 5, 48);
      return 40;
    })()),
    monthlyHoursLimit: Number(employee.monthlyHoursLimit || employee.maxMonthlyHours || (() => {
      const ch = Number(employee.contractHours || 0);
      if (ch === 4) return 87;
      if (ch === 6) return 130;
      if (ch > 0) return Math.round(ch * 174 / 8);
      return 174;
    })()),
...
function getPublishedSchedulePreferences(schedulePreferences) {
  return (schedulePreferences || []).filter((item) => item && item.status !== 'deleted' && item.publishedAt);
}

function schedulePreferenceTimeOffRequests(schedulePreferences) {
  return getPublishedSchedulePreferences(schedulePreferences)
    .filter((item) => item.shiftType === 'Sz' && item.employeeId && item.date)
    .map((item) => ({
      employeeId: item.employeeId,
      startDate: item.date,
      endDate: item.date,
      status: 'accepted',
      reason: 'schedule_preference_time_off',
    }));
}

function buildPreferenceSignals(schedulePreferences) {
  const map = new Map();

  getPublishedSchedulePreferences(schedulePreferences)
...
function preferenceScoreAdjustment({ employee, date, shiftTemplate, preferenceSignals }) {
  const prefs = preferenceSignals?.get(`${employee.id}|${date}`) || [];
  const workPrefs = prefs.filter((item) => item.shiftType !== 'Sz');
  if (workPrefs.length === 0) return 0;

  const templateType = shiftTemplate.key || classifyShiftType(shiftTemplate.startTime, shiftTemplate.endTime);
  const exactMatch = workPrefs.some((item) => {
    const sameTime = item.startTime && item.endTime && item.startTime === shiftTemplate.startTime && item.endTime === shiftTemplate.endTime;
    const sameType = item.shiftType && item.shiftType === templateType;
    return sameTime || sameType;
  });

  return exactMatch ? -4 : 1.5;
}
...
function buildExpectedTemplatesForDate(config, dateKey) {
  ...
  const base = (config.shiftTemplates || []).filter((template) => {
    ...
    // Skip templates that extend beyond the day's opening hours — canAssign would reject all candidates anyway
    if (
      config?.operations?.enforceOpeningHours !== false
      && opening.openTime
      && opening.closeTime
      && !isRangeWithinWindow(template.startTime, template.endTime, opening.openTime, opening.closeTime)
    ) return false;
    return true;
  });
...
export function detectScheduleConflicts({ employees, schedules, vacationRequests = [], schedulePreferences = [], year, month, config }) {
  ...
  const timeOffMap = buildTimeOffMap(normalizedEmployees, [
    ...(vacationRequests || []),
    ...schedulePreferenceTimeOffRequests(schedulePreferences),
  ]);
  ...
  for (let day = 1; day <= monthDays; day += 1) {
    ...
    expectedTemplates.forEach((template) => {
      ...
      const maxStaff = Number(normalizedConfig.maxStaffPerShift || 0);
      if (maxStaff > 0 && shiftsInTemplate.length > maxStaff) {
        pushConflict('warning', 'max_staff', `${date}: túl sok dolgozó a műszakban (${template.startTime}-${template.endTime}, ${shiftsInTemplate.length}/${maxStaff}).`, {
          date,
          shiftType: template.key,
        });
      }

      const maxPharm = Number(normalizedConfig.maxPharmacistsPerShift || 0);
      if (maxPharm > 0 && pharmacists > maxPharm) {
        pushConflict('warning', 'max_pharmacist', `${date}: túl sok gyógyszerész a műszakban (${template.startTime}-${template.endTime}, ${pharmacists}/${maxPharm}).`, {
          date,
          shiftType: template.key,
        });
      }
...
function scoreCandidate({ employee, date, shiftTemplate, employeeStats, preferenceSignals }) {
  const stats = employeeStats.get(employee.id);
  ...
  return loadRatio * 10 + weekendPenalty + nightPenalty + preferencePenalty + preferredShiftBonus + weekdayPreferencePenalty + preferenceScoreAdjustment({
    employee,
    date,
    shiftTemplate,
    preferenceSignals,
...
function scoreCandidateWithNoise({ employee, date, shiftTemplate, employeeStats, preferenceSignals, rand, randomness = 0 }) {
  const baseScore = scoreCandidate({ employee, date, shiftTemplate, employeeStats, preferenceSignals });
    ...
    expectedTemplates.forEach((template) => {
      ...
      for (let i = 0; i < missingStaff; i += 1) {
        const candidates = normalizedEmployees
          ...
          .map((employee) => ({
            employee,
            score: scoreCandidateWithNoise({ employee, date, shiftTemplate: template, employeeStats, preferenceSignals, rand, randomness }),
          }))
...
export function generateAutoSchedulePlan({
  ...
  const preferenceSignals = buildPreferenceSignals(schedulePreferences);
...
export function computePlannerStats({ employees, schedules, vacationRequests = [], schedulePreferences = [], conflicts = [] }) {
  const normalizedEmployees = (employees || []).map(normalizeEmployee);
  const activeSchedules = (schedules || []).filter((item) => item.status !== 'deleted');
  const employeeStats = buildEmployeeStats(normalizedEmployees, activeSchedules);
  ...
  const employeeRows = normalizedEmployees.map((employee) => {
    ...
    const vacationDays = [...(vacationRequests || []), ...schedulePreferenceTimeOffRequests(schedulePreferences)]
