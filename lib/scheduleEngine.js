...
export function detectScheduleConflicts({ employees, schedules, vacationRequests = [], year, month, config }) {
  ...
  normalizedEmployees.forEach((employee) => {
    ...
    for (let i = 1; i < sorted.length; i += 1) {
      ...
      const restHours = dayDiff > 0
        ? ((dayDiff - 1) * 24) + ((24 * 60 - prevEnd + nextStart) / 60)
      ...
      if (normalizedConfig.laborLaw.enforceHungarianLaborLaw && restHours < normalizedConfig.laborLaw.minDailyRestHoursLegal) {
        pushConflict('error', 'legal_rest_time', `${employee.name}: jogszabályi napi pihenőidő sérül (${restHours.toFixed(1)} óra).`, {
          employeeId: employee.id,
          date: next.date,
        });
      }
  ...
  for (let day = 1; day <= monthDays; day += 1) {
    ...
    normalizedConfig.shiftTemplates.forEach((template) => {
    ...
    normalizedConfig.shiftTemplates.forEach((template) => {
      ...
      for (let i = 0; i < missingStaff; i += 1) {
        const candidates = normalizedEmployees
          ...
          .map((employee) => ({
            employee,
            score: scoreCandidate({ employee, date, shiftTemplate: template, employeeStats }),
          }))
