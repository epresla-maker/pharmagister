...
function normalizeEmployee(employee) {
  // Derive weeklyHoursLimit / monthlyHoursLimit from contractHours (napi h) when not explicitly set
  const ch = Number(employee.contractHours || 0);
  let derivedWeekly = 40, derivedMonthly = 174;
  if (ch === 4)       { derivedWeekly = 20;  derivedMonthly = 87;  }
  else if (ch === 6)  { derivedWeekly = 30;  derivedMonthly = 130; }
  else if (ch === 8)  { derivedWeekly = 40;  derivedMonthly = 174; }
  else if (ch === 12) { derivedWeekly = 48;  derivedMonthly = 174; } // shift-worker: same total monthly hours as full-time
  else if (ch > 0)    { derivedWeekly = Math.min(ch * 5, 48); derivedMonthly = Math.round(ch * 174 / 8); }

  return {
    ...employee,
    role: normalizeRole(employee.role),
    weeklyHoursLimit: Number(employee.weeklyHoursLimit || employee.maxWeeklyHours || derivedWeekly),
    monthlyHoursLimit: Number(employee.monthlyHoursLimit || employee.maxMonthlyHours || derivedMonthly),
