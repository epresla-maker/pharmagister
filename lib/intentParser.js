...
export function parseBettiIntent(message, learnedPatterns = []) {
  ...
  if (isChallengeText(text)) {
    return {
      intent: 'challenge_previous',
      confidence: 0.9,
      action: 'challenge_previous',
      entities: { weekday: day, person },
      reply: 'Az elozo pontra reagalva pontositok, nem kezdem ujra a beszelgetest.',
      topCandidates: [{ intent: 'challenge_previous', action: 'challenge_previous', confidence: 0.9, matchType: 'challenge_dictionary', score: 1 }],
      reasoning: buildReasoning('challenge_dictionary', { intent: 'challenge_previous', action: 'challenge_previous', confidence: 0.9 }),
    };
  }
  ...
  const intents = [
    ...
    },
    {
      ...
      action: 'follow_up_decline',
      reply: 'Rendben, akkor nem erre megyunk tovabb. Pontosits, vagy mondok mas lehetoseget.',
    },
    {
      key: 'hesitation',
      confidence: 0.88,
