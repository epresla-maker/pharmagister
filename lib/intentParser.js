...
function semanticIntentFallback(text) {
  ...
  const oneWordDomain = tokens.length === 1 && ['beoszt', 'muszak', 'szabad', 'szabadnap', 'tulora'].includes(tokens[0].stem);
  const confidence = Math.min(0.93, best.confidence + (bestScore - 0.45) * 0.25 + (oneWordDomain ? 0.03 : 0));
...
export function parseBettiIntent(message, learnedPatterns = []) {
  ...
  const intents = [
    ...
    {
      key: 'identity',
      pattern: /(ki nem irta meg.*(tervezet|draft|beoszt)|ki nem irt.*(tervezet|draft|beoszt)|hianyzik.*(tervezet|draft|beoszt)|nincs.*(tervezet|draft|beoszt)|ki nem kuldte be.*(tervezet|draft|beoszt))/, 
    ...
    },
    {
      key: 'thanks',
      confidence: 0.95,
      pattern: /(\bkoszi+\b|\bkoszonom\b|\bkossz\b|\bszuper\b|\brendben\b|\bok\b|\boke\b|\bokay\b|\bthx\b)/,
      action: 'ack',
    ...
    },
    {
      key: 'affirmative',
      confidence: 0.9,
      pattern: /^(igen|ja|persze|oke|ok\b|rendben|johet|mehet|szeretnem|akarom|legyen)\b/,
