...
function isClarifyLastAnswerText(text) {
  const norm = normalizeText(text);
  if (!norm) return false;
...
function semanticIntentCandidates(text) {
  ...
  const hasExplicitEmployeeListVerb = /(mutasd|listazd?|sorold|kik\s+vannak|ki\s+az\s+alkalmazott)/.test(normalizedText);
  ...
  for (const intent of SEMANTIC_INTENTS) {
    if (intent.key === 'list_employees' && !hasExplicitEmployeeListVerb) continue;
...
function semanticIntentFallback(text) {
  ...
  return {
    ...
    confirmed: best.intent === 'list_employees' ? best.confidence > 0.85 : best.intent !== 'identity_check',
    action: best.action,
    reply: best.reply,
...
export function parseBettiIntent(message, learnedPatterns = []) {
  ...
  if (/(alkalmazott\s+vagyok\??|vagyok\s+alkalmazott\??|en\s+alkalmazott\s+vagyok\??|alkalmazottkent\s+vagyok\s+rogzitve\??)/.test(text)) {
    return {
      ...
      topCandidates: [{ intent: 'identity_check', action: 'identity_check', confidence: 0.91, matchType: 'identity_check_priority', score: 1 }],
      reasoning: buildReasoning('identity_check_priority', { intent: 'identity_check', action: 'identity_check', confidence: 0.91 }),
    };
  }
  ...
  if (isClarifyLastAnswerText(text)) {
    return {
      intent: 'clarify_last_answer',
      confidence: 0.93,
      action: 'clarify_last_answer',
      entities: { weekday: day, person },
      reply: 'Ugyanazt egyszerubben elmondom, tema valtas nelkul.',
      topCandidates: [{ intent: 'clarify_last_answer', action: 'clarify_last_answer', confidence: 0.93, matchType: 'clarify_last_answer_dictionary', score: 1 }],
      reasoning: buildReasoning('clarify_last_answer_dictionary', { intent: 'clarify_last_answer', action: 'clarify_last_answer', confidence: 0.93 }),
    };
  }

  if (isChallengeText(text)) {
    return {
      intent: 'challenge_previous_response',
      confidence: 0.9,
      action: 'challenge_previous_response',
      entities: { weekday: day, person },
      reply: 'Az elozo pontra reagalva pontositok, nem kezdem ujra a beszelgetest.',
      topCandidates: [{ intent: 'challenge_previous_response', action: 'challenge_previous_response', confidence: 0.9, matchType: 'challenge_dictionary', score: 1 }],
      reasoning: buildReasoning('challenge_dictionary', { intent: 'challenge_previous_response', action: 'challenge_previous_response', confidence: 0.9 }),
    };
  }
  ...
  if (isNegativeText(text)) {
    return {
      ...
      entities: { weekday: day, person },
      reply: 'Rendben, akkor valtsunk masik iranyra, vagy pontositsuk mire van szukseged.',
  ...
  const intents = [
    ...
    },
    {
      ...
      action: 'follow_up_decline',
      reply: 'Rendben. Pontosits, melyik reszt magyarazzam el mashogy vagy rovidebben.',
    },
    {
      key: 'hesitation',
      confidence: 0.88,
    ...
    {
      key: 'my_schedule_presence',
      confidence: 0.97,
      pattern: /(van\s+beosztasom|lesz\s+beosztasom|nincs\s+beosztasom|van\s+muszakom|lesz\s+muszakom|nincs\s+muszakom|be\s+vagyok\s+osztva\??|be\s+leszek\s+osztva\??)/,
      action: 'check_my_schedule_exists',
      reply: 'Megnezem, van-e beosztasod.',
    },
    {
      key: 'my_schedule',
      confidence: 0.96,
    ...
    },
    {
      key: 'identity_check',
      confidence: 0.91,
      pattern: /(alkalmazott\s+vagyok\??|vagyok\s+alkalmazott\??|en\s+alkalmazott\??)/,
      action: 'identity_check',
      reply: 'Arra gondolsz, hogy alkalmazottkent vagy-e rogzitve a rendszerben, vagy az alkalmazotti listat szeretned latni?',
    },
    {
  ...
  return {
    ...
    confirmed: hit.intent === 'list_employees' ? hit.confidence > 0.85 : hit.intent !== 'identity_check',
    action: hit.action,
    entities: { weekday: day, person },
    reply: hit.reply,
