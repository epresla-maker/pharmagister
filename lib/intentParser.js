      ...
      if (matches.length > 0) {
        ...
        if (pattern.isTimeAware) {
          ...
          if (/most|jelenleg|mennyi.*ido|milyen.*ido/.test(response)) {
            // Replace "11:29" style times with current time
            response = response.replace(/\d{1,2}:\d{2}/g, currentTime);
          }
        }
  ...
  const intents = [
    {
      ...
      action: 'greeting',
      reply: 'Szia! Betti vagyok. Segitek beosztassal, szabadsaggal, tuloraval es ujratervezessel kapcsolatban.',
    },
    {
      key: 'thanks',
      confidence: 0.95,
      pattern: /(koszi|koszonom|szuper|rendben|ok(\b|e))/,
      action: 'ack',
    ...
    },
    {
      key: 'my_schedule',
      confidence: 0.96,
      pattern: /(mi a beosztasom|mikor dolgozom|mikor dolgozok|mikor vagyok muszakban|be vagyok osztva|sajat muszakjaim)/,
    ...
    },
    {
      key: 'report_overtime',
      confidence: 0.95,
      pattern: /(tulora|tuloras|mutasd.*tulora|ki van tuloraban|tuloraban van)/,
