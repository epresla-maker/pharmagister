...
const QUESTION_TERMS = [
  'ki', 'kik', 'kivel', 'kinek', 'mikor', 'hol', 'hogyan', 'miert', 'melyik', 'mennyi', 'hany', 'hanyan', 'meddig', 'mettol', 'mi', 'mit',
];

const ACTION_TERMS = [
  'mutasd', 'listazd', 'sorold', 'tervezd', 'ujratervezes', 'beugrani', 'atvenni', 'szabira', 'szabin',
];

const DOMAIN_SET = new Set(DOMAIN_TERMS);
const SPLIT_MARKERS = [...new Set([...DOMAIN_TERMS, ...QUESTION_TERMS, ...ACTION_TERMS])]
...
function splitConcatenatedToken(tokenNormalized) {
  if (!tokenNormalized || tokenNormalized.length < 10) return null;

  const hasDomainMarker = DOMAIN_TERMS.some((t) => tokenNormalized.includes(t));
  const hasQuestionMarker = QUESTION_TERMS.some((t) => tokenNormalized.includes(t));
  if (!hasDomainMarker && !hasQuestionMarker) return null;

  const chunks = [];
  let i = 0;

  while (i < tokenNormalized.length) {
    const marker = SPLIT_MARKERS.find((m) => tokenNormalized.startsWith(m, i));
    if (marker) {
      chunks.push(marker);
      i += marker.length;
      continue;
    ...
    // Collect unknown span until a known marker starts.
    let j = i + 1;
    while (j < tokenNormalized.length) {
      const nextMarker = SPLIT_MARKERS.some((m) => tokenNormalized.startsWith(m, j));
      if (nextMarker) break;
      j += 1;
    }
    const unknown = tokenNormalized.slice(i, j);
    if (unknown) chunks.push(unknown);
    i = j;
  }

  const markerCount = chunks.filter((c) => SPLIT_MARKERS.includes(c)).length;
  if (chunks.length < 2 || markerCount < 2) return null;
...
export async function normalizeHungarianChatInput(input) {
  ...
  const corrected = parts.map((part) => {
    ...
    const splitVersion = splitConcatenatedToken(normalized);
    if (splitVersion) return splitVersion;
