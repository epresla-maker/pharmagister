...
const MAX_POLICY_VERSIONS = 50;
const MAX_WEIGHT_CHANGE_LOGS = 100;
const MAX_TEMPORARY_MEMORY_ITEMS = 120;
...
const DEFAULT_LONG_TERM_MEMORY = {
  ...
  userModel: {
    communicationStyle: {
      verbosity: 0.45,
      directnessPreference: 0.55,
      emotionalTonePreference: 'medium',
    },
    behaviorPatterns: {
      frequentIntents: [],
      rareIntents: [],
  ...
  stats: {
    ...
    externalValidationSupportCount: 0,
    externalValidationTotalCount: 0,
  ...
  autoTuning: {
    ...
    lastPerformanceDelta: 0,
    lastConfidenceVariance: 0,
    lastPolicyEvolutionScore: 0,
    featureWeights: null,
    safetyEvents: [],
    lastRollbackAt: null,
    safetyOverride: {
  ...
  feedbackClusters: {
    schedule_accuracy_issue: 0,
    role_confusion: 0,
    tone_mismatch: 0,
    contradictory_feedback: 0,
  },
  recentFeedbackWindow: [],
  weightChangeLogs: [],
  policyVersions: [],
...
function mergeLongTermMemory(stored = {}) {
  return {
    ...
    userModel: {
      ...DEFAULT_LONG_TERM_MEMORY.userModel,
      ...(stored.userModel || {}),
      communicationStyle: {
        ...DEFAULT_LONG_TERM_MEMORY.userModel.communicationStyle,
        ...(stored?.userModel?.communicationStyle || {}),
      },
      behaviorPatterns: {
        ...DEFAULT_LONG_TERM_MEMORY.userModel.behaviorPatterns,
        ...(stored?.userModel?.behaviorPatterns || {}),
        frequentIntents: toArray(stored?.userModel?.behaviorPatterns?.frequentIntents).slice(-10),
        rareIntents: toArray(stored?.userModel?.behaviorPatterns?.rareIntents).slice(-10),
      },
      stabilityProfile: {
        ...DEFAULT_LONG_TERM_MEMORY.userModel.stabilityProfile,
        ...(stored?.userModel?.stabilityProfile || {}),
      },
    },
    conversationState: {
      ...DEFAULT_LONG_TERM_MEMORY.conversationState,
      ...(stored.conversationState || {}),
      activeTopic: {
        ...DEFAULT_LONG_TERM_MEMORY.conversationState.activeTopic,
        ...(stored?.conversationState?.activeTopic || {}),
        requiredSlots: toArray(stored?.conversationState?.activeTopic?.requiredSlots).slice(-10),
        filledSlots: toArray(stored?.conversationState?.activeTopic?.filledSlots).slice(-10),
      },
      openLoops: toArray(stored?.conversationState?.openLoops).slice(-6).map((loop) => ({
        question: loop?.question || '',
        expectedAnswer: loop?.expectedAnswer || null,
        awaiting: Boolean(loop?.awaiting),
        status: loop?.status || 'open',
        age: Number(loop?.age || 0),
      })),
      intentStack: toArray(stored?.conversationState?.intentStack).slice(-8),
    },
    sessionMemory: toArray(stored.sessionMemory).slice(-12),
    topicMemory: stored.topicMemory || null,
    assistantMemory: {
      ...DEFAULT_LONG_TERM_MEMORY.assistantMemory,
      ...(stored.assistantMemory || {}),
      policy: {
        ...DEFAULT_LONG_TERM_MEMORY.assistantMemory.policy,
        ...(stored?.assistantMemory?.policy || {}),
      },
      permanentProfile: {
        ...DEFAULT_LONG_TERM_MEMORY.assistantMemory.permanentProfile,
        ...(stored?.assistantMemory?.permanentProfile || {}),
      },
      temporaryNotes: toArray(stored?.assistantMemory?.temporaryNotes).slice(-MAX_TEMPORARY_MEMORY_ITEMS).map((item) => ({
      ...
      lastPrunedAt: stored?.assistantMemory?.lastPrunedAt || null,
    },
    longTermFacts: toArray(stored.longTermFacts || stored.stableFacts).slice(-50),
    ...
    stats: {
      ...
      errorTaxonomy: {
        ...DEFAULT_LONG_TERM_MEMORY.stats.errorTaxonomy,
        ...(stored?.stats?.errorTaxonomy || {}),
      },
    },
    autoTuning: {
      ...DEFAULT_LONG_TERM_MEMORY.autoTuning,
      ...(stored.autoTuning || {}),
      safetyEvents: toArray(stored?.autoTuning?.safetyEvents).slice(-20),
      safetyOverride: {
        ...DEFAULT_LONG_TERM_MEMORY.autoTuning.safetyOverride,
        ...(stored?.autoTuning?.safetyOverride || {}),
      },
    },
    ...
    performance: {
      ...
      recentScoreHistory: toArray(stored?.performance?.recentScoreHistory).slice(-200),
    },
    feedbackClusters: {
      ...DEFAULT_LONG_TERM_MEMORY.feedbackClusters,
      ...(stored.feedbackClusters || {}),
    },
    recentFeedbackWindow: toArray(stored.recentFeedbackWindow).slice(-80),
    weightChangeLogs: toArray(stored.weightChangeLogs).slice(-MAX_WEIGHT_CHANGE_LOGS),
    policyVersions: toArray(stored.policyVersions).slice(-MAX_POLICY_VERSIONS),
    activePolicyVersion: stored.activePolicyVersion || null,
    stability: {
      ...DEFAULT_LONG_TERM_MEMORY.stability,
      ...(stored.stability || {}),
    },
    externalValidationHistory: toArray(stored.externalValidationHistory).slice(-80),
    strategyMetrics: (stored?.strategyMetrics && typeof stored.strategyMetrics === 'object') ? stored.strategyMetrics : {},
    strategyMemory: (stored?.strategyMemory && typeof stored.strategyMemory === 'object') ? stored.strategyMemory : {},
    integrity: {
      ...DEFAULT_LONG_TERM_MEMORY.integrity,
      ...(stored?.integrity || {}),
    },
