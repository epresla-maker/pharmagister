...
function handleScheduleOps({ parsed, chatRole }) {
  const actionMap = {
    ...
    find_replacement: { action: 'find_replacement', reply: 'Rendben, segitek a helyettesitesi igenyekkel is. Megnezem, ki tudna atvenni vagy beugrani a muszakra.' },
  };
...
function handleGreeting({ parsed, chatRole, context }) {
  const firstName = String(context?.userName || '').trim();
  const greetingLead = firstName ? `Szia, ${firstName}!` : 'Szia!';
  return {
    action: 'greeting',
    reply: `${greetingLead} Betti vagyok, a Pharmagister AI asszisztense. Miben segithetek?`,
...
function handleThanks({ parsed, chatRole, context }) {
  const firstName = String(context?.userName || '').trim();
  const lead = firstName ? `${firstName}, szivesen!` : 'Szivesen!';
  return {
    action: 'ack',
    reply: `${lead} Ha szeretned, mar most megmutatom a kovetkezo muszakjaidat vagy szabadnapjaidat.`,
    quickActions: buildUnknownSuggestions('', chatRole),
    entities: parsed.entities,
  };
}
...
function clarifyWithTopicContinuity({ parsed, conversationState, chatRole, message }) {
  ...
  if (activeTopic && activeTopic !== 'general' && activeTopic !== 'clarify_with_options') {
    // If there's an open loop question already, restate it once (age === 0).
    // If it's already been asked (age > 0), drop the loop and give a fresh response.
    if (openLoop?.question && Number(openLoop?.age || 0) === 0) {
      const reply = `Meg mindig erre varok: ${openLoop.question}`;
      return {
        action: 'clarify_with_options',
        reply,
        quickActions: buildUnknownSuggestions(message, chatRole),
...
export function runBettiPipeline({
  ...
  // ── STEP 0: GREETING RESET ────────────────────────────────────────────────
  // If user sends a greeting, clear all open loops and active topic.
  const msgNormGreet = String(message || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim();
  const isGreeting = /^(halo|hello|szia|helló|üdv|udvozlom|jo reggelt|jo napot|jo estet|hey|hi|üdvözlöm)$/.test(msgNormGreet);
  if (isGreeting) {
    const cleanState = { ...buildDefaultConversationState() };
    return {
      action: 'greeting',
      reply: 'Szia! Miben segíthetek ma?',
      quickActions: buildUnknownSuggestions(message, chatRole),
      entities: {},
      payload: { action: 'greeting', entities: {}, confidence: 1 },
      nextConversationState: cleanState,
    };
  }
