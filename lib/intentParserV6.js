...
export function classifyIntent(message, context = {}) {
  ...
  const scores = {
    identity_check: scoreCategory(text, IDENTITY_CHECK_PATTERNS, 0.90),
    data_query: scoreCategory(text, DATA_QUERY_PATTERNS, 0.88),
    schedule_ops: scoreCategory(text, SCHEDULE_OPS_PATTERNS, 0.86),
    modification_request: scoreCategory(text, MODIFICATION_PATTERNS, 0.85),
    clarification: scoreCategory(text, CLARIFICATION_PATTERNS, 0.82),
  };
