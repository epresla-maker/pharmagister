...
const SYNONYM_REPLACEMENTS = [
  [/\b(szabi|szabin|szabira[a-z0-9]*|szabadsagra)\b/g, 'szabadsag'],
...
const GREETING_PATTERNS = [
  /^(szia|sziasztok|szio|szio|hello|helo|helló|hali|hay|hey|hi|jo napot|jo reggelt|jo estet|jo ejszakat|jo ejt|udvozlom|udv|pacsi)([\s!.]*|$)/,
];
...
const SCHEDULE_OPS_PATTERNS = [
  ...
  /(ki tudna atvenni|atvenni.*muszak|helyettesitis|helyettesites|helyettesitesi igeny|helyettesitesi igenyek|ki tud beugrani|ki tudna beugrani|beugrani|beugrana|potolna|potolni)/,
...
const QUESTION_WORDS = [
  'ki', 'kik', 'kivel', 'kinek', 'mikor', 'hol', 'hogyan', 'miert', 'melyik', 'mennyi', 'hany', 'hanyan', 'meddig', 'mettol',
  'mi', 'mit',
];

const QUESTION_START_PATTERNS = [
  /^(ki|kik|kivel|kinek|mikor|hol|hogyan|miert|melyik|mennyi|hany|hanyan|meddig|mettol|mi|mit)\b/,
  /^(szeretnem tudni|meg tudod mondani|el tudod mondani|mond meg)\b/,
];

const CATEGORY_HINT_TOKENS = {
  identity_check: ['ki', 'vagy', 'alkalmazott', 'profil', 'azonositas'],
  data_query: ['beosztas', 'muszak', 'szabadsag', 'szabadnap', 'tulora', 'dolgozo', 'alkalmazott', 'tervezet', 'lista', 'mutasd', 'mikor'],
  schedule_ops: ['ujratervezes', 'helyettesites', 'beugr', 'atven', 'potol', 'fix', 'lock', 'minimalis'],
  modification_request: ['igazsagos', 'fair', 'kevesebb', 'csokkent', 'modosits', 'optimalizal'],
  clarification: ['igen', 'nem', 'talan', 'segits', 'nem ertem'],
};
...
export function classifyIntent(message, context = {}) {
  ...
  const questionSignal = hasQuestionSignal(raw, text, tokens);
  ...
  // SPECIAL CASE: pure greeting — must run before identity check
  if (GREETING_PATTERNS.some((p) => p.test(text))) {
    return {
      category: 'greeting',
      confidence: 0.97,
      action: 'greeting',
      reply: 'Szia! Betti vagyok, a Pharmagister AI asszisztense. Miben segithetek?',
      needsDisambiguation: false,
      alternatives: [],
      entities: { weekday },
    };
  ...
  // Use higher baseScores to reflect actual confidence when patterns match
  const scores = {
    identity_check: scoreCategory(text, IDENTITY_CHECK_PATTERNS, 0.90, { tokens, questionSignal, category: 'identity_check' }),
    data_query: scoreCategory(text, DATA_QUERY_PATTERNS, 0.88, { tokens, questionSignal, category: 'data_query' }),
    schedule_ops: scoreCategory(text, SCHEDULE_OPS_PATTERNS, 0.86, { tokens, questionSignal, category: 'schedule_ops' }),
    modification_request: scoreCategory(text, MODIFICATION_PATTERNS, 0.85, { tokens, questionSignal, category: 'modification_request' }),
    clarification: scoreCategory(text, CLARIFICATION_PATTERNS, 0.82, { tokens, questionSignal, category: 'clarification' }),
  };
 ...
 * Logic: If ANY pattern matches, assume high confidence (≥ 0.80)
 * This is more lenient than requiring 100% pattern coverage
 */
function hasQuestionSignal(rawText, normalizedText, tokens) {
  if (String(rawText || '').includes('?')) return true;
  if (QUESTION_START_PATTERNS.some((p) => p.test(normalizedText || ''))) return true;
  if ((tokens || []).some((t) => QUESTION_WORDS.includes(t))) return true;
  // Hungarian yes/no style forms: "van-e", "lehet-e", "kell-e", stb.
  if (/\b[a-z0-9]+-e\b/.test(normalizedText || '')) return true;
  return /\b(ki|kik|mikor|hol|hogyan|miert|melyik|mennyi|hany|meddig|mettol|mi|mit)\b/.test(normalizedText || '');
}

function hasCooccurrence(text, groupA, groupB) {
  const hasA = groupA.some((w) => text.includes(w));
  const hasB = groupB.some((w) => text.includes(w));
  return hasA && hasB;
}

function scoreSemanticHints(text, tokens, category, questionSignal) {
  const hints = CATEGORY_HINT_TOKENS[category] || [];
  if (!hints.length) return 0;

  const overlap = (tokens || []).filter((token) => hints.some((h) => token.includes(h) || h.includes(token))).length;
  let score = Math.min(0.15, overlap * 0.03);

  if (questionSignal && (category === 'data_query' || category === 'schedule_ops' || category === 'identity_check')) {
    score += 0.03;
  }

  if (category === 'data_query' && hasCooccurrence(text, ['ki', 'kik', 'mikor', 'mutasd', 'lista'], ['beosztas', 'szabadsag', 'szabadnap', 'tulora', 'tervezet', 'dolgozo'])) {
    score += 0.08;
  }

  if (category === 'schedule_ops' && hasCooccurrence(text, ['ki', 'kik', 'tud', 'tudna'], ['atven', 'beugr', 'helyettes', 'potol'])) {
    score += 0.10;
  }

  if (category === 'modification_request' && hasCooccurrence(text, ['legyen', 'szeretnem', 'szeretnek', 'csokkentsd', 'kevesebb'], ['igazsagos', 'fair', 'tulora', 'optimalizal'])) {
    score += 0.08;
  }
...
function scoreCategory(text, patterns, baseScore, context = {}) {
  const { tokens = [], questionSignal = false, category = '' } = context;
  const matches = patterns.filter((p) => p.test(text)).length;
  const semanticScore = scoreSemanticHints(text, tokens, category, questionSignal);

  if (matches === 0) {
    // No direct regex match: require stronger multi-word/semantic evidence.
    if (semanticScore < 0.12) return 0;
    return Math.min(0.84, 0.66 + semanticScore);
  }
  
  // If at least one pattern matches, return high confidence score
  // More matches increase confidence, but even one match is strong
  const confidenceBoost = Math.min(0.15, (matches - 1) * 0.05);
  return Math.min(0.99, baseScore + confidenceBoost + semanticScore);
}
...
function getActionForCategory(category, text) {
  ...
  switch (category) {
    ...
    case 'data_query':
      ...
      if ((/\bki(k)?\b/.test(textLower) || textLower.includes('igeny')) && textLower.includes('szab')) return 'show_vacation_requests';
    ...
    case 'schedule_ops':
      ...
      if (textLower.includes('helyettes') || textLower.includes('atven') || textLower.includes('beugr') || textLower.includes('potol')) return 'find_replacement';
...
function getReplyForCategory(category, text) {
  ...
  switch (category) {
    ...
    case 'data_query':
      ...
      if ((/\bki(k)?\b/.test(textLower) || textLower.includes('igeny')) && textLower.includes('szab')) {
        return 'Rendben, megmutatom a szabadsagigenyeket.';
      }
  ...
  switch (category) {
    ...
    case 'data_query':
      switch (action) {
        ...
        case 'show_vacation_requests': return 'show_vacation_requests';
