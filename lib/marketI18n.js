...
export function getClientMarket() {
  ...
  if (match?.[1]) {
    return normalizeMarket(decodeURIComponent(match[1]));
  }
  ...
  if (typeof navigator !== 'undefined') {
    const locales = Array.isArray(navigator.languages) && navigator.languages.length > 0
      ? navigator.languages
      : [navigator.language || ''];
    const hasGerman = locales.some((locale) => String(locale || '').toLowerCase().startsWith('de'));
    return hasGerman ? 'de' : 'hu';
  }
...
export function getLocalizedDemandPositionLabel(position, market = 'hu', legacyLabel = '') {
  const m = normalizeMarket(market);
  const normalizedPosition = String(position || '').toLowerCase();
  const normalizedLegacyLabel = String(legacyLabel || '').toLowerCase();

  let canonical = normalizedPosition;
  if (!['pharmacist', 'assistant', 'pka'].includes(canonical)) {
    if (normalizedLegacyLabel.includes('apotheker') || normalizedLegacyLabel.includes('gyogyszeresz') || normalizedLegacyLabel.includes('gyógyszerész')) {
      canonical = 'pharmacist';
    } else if (normalizedLegacyLabel.includes('pta') || normalizedLegacyLabel.includes('assistent') || normalizedLegacyLabel.includes('asszisztens') || normalizedLegacyLabel.includes('szakasszisztens')) {
      canonical = 'assistant';
    } else if (normalizedLegacyLabel.includes('pka')) {
      canonical = 'pka';
    }
  }

  if (canonical === 'pharmacist') {
    return m === 'de' ? 'Apotheker/in' : 'Gyógyszerész';
  }
  if (canonical === 'assistant') {
    return m === 'de' ? 'PTA' : 'Szakasszisztens';
  }
  if (canonical === 'pka') {
    return m === 'de' ? 'PKA' : 'PKA (Gyógyszerészeti kereskedő)';
  }
  return m === 'de' ? 'Mitarbeiter/in' : 'Munkatárs';
}
