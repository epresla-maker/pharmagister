...
const BETTI_DOMAIN_CLASSIFIER_PROMPT = `Te egy nagyon olcsó, rövid döntő osztályozó vagy a Pharmagister számára.

Feladat:
- Döntsd el, hogy a felhasználó üzenete Pharmagisterhez, műszakhoz, beosztáshoz, szabadsághoz, túlórához, helyettesítéshez, dolgozókhoz vagy gyógyszertári munkarendhez kapcsolódik-e.
- Ha IGEN, válaszolj pontosan ezzel: DOMAIN
- Ha NEM, és általános small talk, világismeret, időjárás, vers, programozás, recept, vicc, politika, sport, idő, általános tanács vagy egyéb nem-Pharmagister téma, válaszolj pontosan ezzel: OFFTOPIC
- Ha nem teljesen egyértelmű, de inkább szakmai/szoftveres munkaszervezési kérdés, válaszolj DOMAIN

Csak egy szót adhatsz vissza: DOMAIN vagy OFFTOPIC`;
...
function extractClassifierLabel(text) {
  const upper = String(text || '').trim().toUpperCase();
  if (upper.includes('OFFTOPIC')) return 'OFFTOPIC';
  if (upper.includes('DOMAIN')) return 'DOMAIN';
  return null;
}

export async function classifyBettiDomain({ message, chatRole, userName = null }) {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) {
    return { decision: 'DOMAIN', usedClassifier: false, error: 'GEMINI_API_KEY nincs beállítva' };
  }
  ...
  try {
    ...
    const model = genAI.getGenerativeModel({
      ...
      systemInstruction: BETTI_DOMAIN_CLASSIFIER_PROMPT,
      generationConfig: {
        maxOutputTokens: 8,
        temperature: 0,
      },
    ...
    const prompt = [
      userName ? `Felhasználó keresztneve: ${userName}` : null,
      `Felhasználó szerepköre: ${roleLabel}`,
      `Üzenet: ${message}`,
    ].filter(Boolean).join('\n');

    const result = await model.generateContent(prompt);
    const label = extractClassifierLabel(result.response.text());
    if (!label) {
      return { decision: 'DOMAIN', usedClassifier: true, error: 'Érvénytelen classifier válasz' };
    }

    return { decision: label, usedClassifier: true };
  } catch (err) {
    return { decision: 'DOMAIN', usedClassifier: false, error: err.message || 'Classifier hiba' };
  }
}
